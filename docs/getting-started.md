...
- **KiCad 10** (`kicad-cli` + its bundled python with `pcbnew`) — for `text-to-pcb`.
- A **build123d** venv (`python3 -m venv .venv && .venv/bin/pip install build123d`) — for `text-to-cad`.
- Your firmware toolchain (ESP-IDF / PlatformIO / Arduino-CLI) — for `text-to-firmware`.
...
[`text-to-pcb` spec template](../skills/text-to-pcb/references/spec-template.md) is the
